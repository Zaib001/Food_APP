...
import StockBarChart from "./StockBarChart"; // <- add this
...
export default function StockCheck() {
  ...
  const handleSave = (item) => {
    ...
    const ingredientName =
      ingredients.find((i) => (i._id ?? i.id) === item.ingredientId)?.name ||
      item.ingredientId;

    const itemWithName = { ...item, quantity: qty, ingredientName };

    if (editIndex !== null) {
      setInventory((prev) => {
        const copy = [...prev];
        copy[editIndex] = itemWithName;
        return copy;
      });
      setEditIndex(null);
    } else {
  ...
  return (
    <div className="p-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/80 text-sm">
                Capture deliveries and track stock health.
              </p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl bg-white/15 border border-white/20 p-3 text-center">
              <div className="text-[11px] text-white/80">Items</div>
              <div className="text-lg font-extrabold">{stats.totalItems}</div>
            </div>
            <div className="rounded-xl bg-white/15 border border-white/20 p-3 text-center">
            ...
            <div className="rounded-xl bg-white/15 border border-white/20 p-3 text-center">
              <div className="text-[11px] text-white/80">Low Stock</div>
      ...
      {/* Optional: Chart */}
      <div className="mt-6 rounded-2xl border border-gray-100 bg-white/80 shadow-sm p-4">
