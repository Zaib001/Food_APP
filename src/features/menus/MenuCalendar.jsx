              ...
              {mealOrder.map((type) => {
                ...
                return (
                  <div key={type} className="border rounded-lg p-3">
                    <h3 className="text-md font-semibold capitalize mb-2 text-red-600">{type}</h3>
                    <ul className="text-sm text-gray-700 space-y-1">
                      <li className="flex items-center gap-2">
                        <FaDrumstickBite className="text-red-500" />
                        Protein(s): {menu.proteins?.map(getName).join(', ')}
                      </li>
                      <li className="flex items-center gap-2">
                        <FaLeaf className="text-green-600" />
                        Side(s): {menu.sides?.map(getName).join(', ')}
                      </li>
                      <li className="flex items-center gap-2">
                        <FaBreadSlice className="text-yellow-600" />
                        Bread(s): {menu.breads?.map(getName).join(', ')}
                      </li>
                      <li className="flex items-center gap-2">
                        <FaMugHot className="text-purple-500" />
                        Beverage(s): {menu.beverages?.map(getName).join(', ')}
                      </li>
                      <li className="flex items-center gap-2">
