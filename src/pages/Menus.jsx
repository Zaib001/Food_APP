...
export default function Menus() {
  ...
  const generateRequisitionsFromMenus = () => {
    ...
    filteredMenus.forEach(menu => {
      const allItems = [menu.protein, ...menu.sides, ...menu.breads, ...menu.beverages];
      allItems.forEach(id => {
        if (id && ingredientsMap[id]) {
          const key = `${id}|${menu.date}|${menu.base}`;
          ingredientQuantities[key] = (ingredientQuantities[key] || 0) + 1;
        }
      });
    });

    const newRequisitions = Object.entries(ingredientQuantities).map(([key, quantity]) => {
      const [id, date, base] = key.split('|');
